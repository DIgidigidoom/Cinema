...
function showSeatDetails(pos) {
    ...
    elPopup.querySelector('h4 span').innerText = countAvailableSeatsAround(gCinema, pos.i, pos.j, pos)
...
function countAvailableSeatsAround(board, rowIdx, colIdx) {
    ...
    console.log(gHlSeats)
    return count
}

function highlightNegSeats(hlSeats) {
    const elPopup = document.querySelector('.popup')
    const elBtn = elPopup.querySelector('.btn-book-seat')
    const i = +elBtn.dataset.i
    const j = +elBtn.dataset.j
    //debugger
    countAvailableSeatsAround(gCinema, i, j)
    if (!gIsHl) {
        addHighlight(hlSeats)
        gHlTimeout = setTimeout(() => removeHighlight(hlSeats), 2500)
    } else {
        removeHighlight(hlSeats)
    }
}

function addHighlight(hlSeats) {
    gIsHl = true
    for (var i = 0; i < hlSeats.length; i++) {
        var elHlseat = document.querySelector(`[title="Seat: ${hlSeats[i].i + 1}, ${hlSeats[i].j + 1}"]`)
        elHlseat.classList.add('highlighted')
        console.log(elHlseat)
    }

}
function removeHighlight(hlSeats) {
    gIsHl = false
    clearTimeout(gHlTimeout)
    for (var i = 0; i < hlSeats.length; i++) {
        var elHlseat = document.querySelector(`[title="Seat: ${hlSeats[i].i + 1}, ${hlSeats[i].j + 1}"]`)
        elHlseat.classList.remove('highlighted')
        console.log(elHlseat)
    }
    gHlSeats = []
